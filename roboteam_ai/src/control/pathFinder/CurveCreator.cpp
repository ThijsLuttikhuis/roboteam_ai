...
void CurveCreator::createCurve(std::vector<Vector2> pathNodes, std::vector<Vector2> robotCoordinates, float startVelocity, float endVelocity) {
...
void CurveCreator::createCurvePiece(std::vector<Vector2> pathPiece,
    ...
    if (pathPiece.size() == 1) {
        ...
        controlPoints.push_back(pathPiece[0] + (pathPiece[1]-pathPiece[0]).scale(1)); // TODO: .scale() could be used to minimize curvature
        ...
        for (int i=0; i < pathPiece.size()-2; i++) {
            ...
            if (mostDangerousObstacle.empty()) {
                controlPoints.push_back(pathPiece[i+1] + (pathPiece[i+2]-pathPiece[i+1]).scale(1)); // TODO: .scale() could be used to minimize curvature
            } else {
                // calculate control point
                Vector2 maxOfControlPoint = pointOnLinePastObstacle(dangerZone[0], mostDangerousObstacle[0], {dangerZone[1], dangerZone[2]});
                controlPoints.push_back(pathPiece[i+1] + (maxOfControlPoint-pathPiece[i+1]).scale(1)); // TODO: .scale() could be used to minimize curvature
                return;
            }
        }
    }
...
std::vector<Vector2> CurveCreator::findMostDangerousObstacle(std::vector<Vector2> trianglePoints, std::vector<Vector2> robotCoordinates) {
    ...
    for (Vector2 &obstaclePos: robotCoordinates) {
        if (isObstacleInTriangle(trianglePoints, obstaclePos)) {
            if (mostDangerousObstacle.empty()) {
                ...
                float anglePathToCurrentObstacle = (float)abs(pathLine.angle() - startToCurrentObstacle.angle()) - (float)atan(robotDiameter/startToCurrentObstacle.length());
                float anglePathToNewObstacle = (float)abs(pathLine.angle() - startToNewObstacle.angle()) - (float)atan(robotDiameter/startToNewObstacle.length());
...
bool CurveCreator::isObstacleInTriangle(std::vector<Vector2> trianglePoints, Vector2 obstaclePos) {
    ...
    float sumOfAngles = 0;
    sumOfAngles += acos((trianglePoints[0] - obstaclePos).normalize().dot((trianglePoints[1] - obstaclePos).normalize()));
    sumOfAngles += acos((trianglePoints[1] - obstaclePos).normalize().dot((trianglePoints[2] - obstaclePos).normalize()));
    sumOfAngles += acos((trianglePoints[2] - obstaclePos).normalize().dot((trianglePoints[0] - obstaclePos).normalize()));
...
Vector2 CurveCreator::pointOnLinePastObstacle(Vector2 startPoint, Vector2 obstaclePos,
    ...
    double scaleFactor = (startPoint.dot(helpVector) - linepoints[0].dot(helpVector)) / (linepoints[1].dot(helpVector) - linepoints[0].dot(helpVector));
...
void CurveCreator::calculateControlPoints(std::vector<Vector2> pathNodes, std::vector<Vector2> &robotCoordinates, float startVelocity, float endVelocity) {
    if (pathNodes.size() == 2) {
        ...
        if (curvePieceEdgeIndices.size() > 2) {
            ...
            for (int j=0; j<curvePieceEdgeIndices.size()-1; j++) {
                std::vector<Vector2> curve2(controlPoints.begin()+curvePieceEdgeIndices[j], controlPoints.begin()+curvePieceEdgeIndices[j+1]+1);
...
void CurveCreator::addVelocityControlPoints(float startVelocity, float endVelocity, int numberOfCurvePieces) {
    ...
    endScaleFactor = endScaleFactor > (float)(controlPoints[controlPoints.size()-2]-controlPoints[controlPoints.size()-1]).length()
            ? (float)(controlPoints[controlPoints.size()-2]-controlPoints[controlPoints.size()-1]).length() : endScaleFactor;
    Vector2 endVelCP = controlPoints.back() + (controlPoints[controlPoints.size()-2]-controlPoints[controlPoints.size()-1]).stretchToLength(endScaleFactor);
...
void CurveCreator::calculateAcceleration() {
    ...
    curveAccelerations.back() = curveAccelerations[curveAccelerations.size()-2] + (curveAccelerations[curveAccelerations.size()-2]-curveAccelerations[curveAccelerations.size()-3]);
}
